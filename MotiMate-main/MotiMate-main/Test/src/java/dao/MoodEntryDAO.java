...
public class MoodEntryDAO {
    ...
    public boolean updateEntry(MoodEntry entry) {
        // If custom mood is provided, validate and convert to mood_id
        if (entry.getCustomMood() != null && !entry.getCustomMood().trim().isEmpty()) {
            if (!isValidMoodName(entry.getCustomMood())) {
                return false; // Invalid mood name
        ...
        String sql = "UPDATE MOOD_ENTRIES SET MOOD_ID = ?, CUSTOM_MOOD = ?, ENTRY_TITLE = ?, JOURNAL_ENTRY = ? WHERE ENTRY_ID = ? AND USER_ID = ?";
        
        try (Connection conn = DBConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            
            if (entry.getMoodId() != null) {
                stmt.setInt(1, entry.getMoodId());
            } else {
                stmt.setNull(1, Types.INTEGER);
            }
            stmt.setString(2, entry.getCustomMood());
            stmt.setString(3, entry.getEntryTitle());
            stmt.setString(4, entry.getJournalEntry());
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    if (moodName != null) {
                        ...
                        entry.setDisplayEmoji("📝"); // Default emoji for custom moods
                    }
